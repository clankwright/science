...
- [x] 13.5 [easy] [should-fix] `SKILL.md:908` — Mode B.1's "Note:" list omits the `profile:` declaration, so agents reading schema specs that now include `profile:` (written by the fixed A.3 template) have no explicit instruction to apply §The profile axis defaults (link style, front-matter requirements, license tracking) during ingest passes. Mode C.1 inherits the same gap via "Same as B.1." Without this hook, the feature's stated purpose — "the profile signals intent so the agent doesn't ask about link style, front-matter requirements, and license tracking on every ingest" — is not reliably realized in Mode B/C operations. Proposed fix: append `profile: (if present, apply §The profile axis defaults for link style and front-matter requirements)` to the "Note:" items in Mode B.1 (line 908).
...
- [x] 14.1 [medium] Run `sst-sanitize-transferable` over the working SKILL.md; resolve any `must-fix` findings (proprietary leakage, banned terms). `nit` findings are reviewed and either accepted or rewritten.
- [x] 14.2 [easy] Invoke `/sst-promote-skill-proposal` to copy the sanitized SKILL.md over the canonical at `~/Dev/skill-set/skills/research/sst-wiki-curator/SKILL.md`. Bump the `version:` frontmatter field (1.0.1 → 1.1.0 since the schema gained new page kinds, profile axis, umbrella mode).
- [x] 14.3 [easy] Commit the promotion in `~/Dev/skill-set/` with an imperative-mood subject; verify `bin/validate-frontmatter.py` passes; run `bin/install-skills.sh -y --force` so `~/.claude/skills/sst-wiki-curator/` picks up the new version.
- [x] 14.4 [easy] Final science-side commit closing 14.1-14.3 in `docs/SPEC.md` + `docs/TODO.md`; archive this spec as `docs/SPEC-archive-wiki-curator-evolution.md` if you want to keep the testbed working directory clean for the next skill-evolution project.

**Phase 14 completed 2026-05-23.** Applied all must-fix and should-fix sanitization findings from the existing findings file (5 must-fix: frontmatter proprietary fields and `~/Dev/science/` absolute paths; 9 should-fix: `biology/longevity/`, `comsci/edge-llm/`, `comsci/ai-empowerment/` testbed path prefixes, specific project examples in §Adjacent patterns, `bpu/wiki/index.md` testbed reference, and `aliens/` paths in worked examples; 1 nit: covered by the must-fix rewrite). Wrote the sanitized canonical directly to `~/Dev/skill-set/skills/research/sst-wiki-curator/SKILL.md` with updated frontmatter: `name: sst-wiki-curator`, `version: 1.1.0`, `argument-hint` extended with `umbrella <parent-dir>`, `transferable:` and `transferable-version:` fields removed. `bin/validate-frontmatter.py` passed; `bin/install-skills.sh -y --force` installed the updated skill. Archived this spec as `docs/SPEC-archive-wiki-curator-evolution.md`.

- Changes: `~/Dev/skill-set/skills/research/sst-wiki-curator/SKILL.md` (promoted v1.1.0), `docs/SPEC.md`, `docs/TODO.md`, `docs/SPEC-archive-wiki-curator-evolution.md` (archive).
- Sanitize: must-fix=0 (all resolved in the promoted canonical).
- Test delta: n/a (prose skill, no automated tests).
